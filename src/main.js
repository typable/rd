...
customElements.define('rd-viewer', Viewer);

const viewer = document.createElement('rd-viewer');
...
style.innerHTML = `
  ...
  body.reader-mode > *:not(.read-container) {
...
linkA.href= 'https://fonts.googleapis.com/css2?family=Source+Serif+Pro:ital,wght@0,400;0,600;1,400;1,600&display=swap';

const linkB = document.createElement('link');
linkB.rel = 'stylesheet';
linkB.href= 'https://fonts.googleapis.com/css2?family=JetBrains+Mono:ital,wght@0,400;0,500;1,400;1,500&display=swap';
...
function init() {
  ...
  tags.forEach((tag) => list.push(...document.querySelectorAll(`${tag}:not(rd-viewer *)`)));
  ...
  viewer.shadow.appendChild(style);
  document.head.appendChild(linkA);
  document.head.appendChild(linkB);
  viewer.shadow.appendChild(container);
